...
sudo chmod 644 s3-drive/nginx_https.conf
...
sudo cp /home/ubuntu/s3-drive/nginx_https.conf /etc/nginx/sites-available/

# making soft link to nginx config file from "sites-available" directory to "sites-enabled" directory
sudo ln -s /etc/nginx/sites-available/nginx_https.conf /etc/nginx/sites-enabled/default

# making lets encrypt ssl certeficate and configure nginx to redirect http to https
sudo certbot --nginx --register-unsafely-without-email --agree-tos --redirect -d artur-tshitoyan.acadevopscourse.xyz

# Restart nginx
sudo systemctl restart nginx

## Create the ssl renew script in crontab
sudo su -c 'echo "0 5 * * * /usr/bin/certbot renew --quiet" >> /var/spool/cron/crontabs/ubuntu'

## Copy ssl certeficate with key to mounted s3 bucket
sudo su -c "cp /etc/letsencrypt/archive/artur-tshitoyan.acadevopscourse.xyz/* /home/ubuntu/s3-drive/"
