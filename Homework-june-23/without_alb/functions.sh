...
make_A_record_create_file ()
  {
echo '{
            "Comment": "artur-tshitoyan.acadevopscourse.xyz",
            "Changes": [{
            "Action": "CREATE",
                        "ResourceRecordSet": {
                                    ...
                                    "Type": "A",
                                    "TTL": 300,
                                 "ResourceRecords": [{ "Value": "'$AWS_EC2_INSTANCE_PUBLIC_IP'"}]
}}]
}' > ./create_A_record.json
  }
...
add_A_record ()
  {
HOSTED_ZONE_CHANGE_ID=$(aws route53 change-resource-record-sets \
--hosted-zone-id $HOSTED_ZONE_ID \
--change-batch file://create_A_record.json \
--output text | awk '{print$3}')
AWS_VPC="$AWS_VPC $HOSTED_ZONE_CHANGE_ID"
  }
...
delete_S3 ()
  ...
  rm -f index.html nginx_https.conf && \
  delete_custom_security_group
  }
...
--hosted-zone-id $HOSTED_ZONE_ID \
--change-batch file://delete_A_record.json && \
