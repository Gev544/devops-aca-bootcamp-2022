...
aws ec2 associate-route-table  --subnet-id $subnet1_ID --route-table-id $rtID >> IDs.txt
...
aws ec2 create-security-group --group-name my-homework-SG --description "SG for homework SSH access" --vpc-id $vpcID >> IDs.txt
sgID=`cat IDs.txt |tail -2|head -1| cut -d '"' -f 4`
