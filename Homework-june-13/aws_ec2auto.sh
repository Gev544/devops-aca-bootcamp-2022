...
aws ec2 create-tags \
        --resources $vpc \
        --tags Key=Name,Value=My_Vpc_Kyesayan
...
then
	echo "gateway_from_vpc is created"
else
...
then
	echo "routeTable_for_vpc is created"
else
...
# add route_for_all_traffic
route_for_all_traffic=$(aws ec2 create-route \
        --route-table-id $routeTable_for_vpc \
        --destination-cidr-block 0.0.0.0/0 \
        --gateway-id $igw)

if [[ ! -s $route_for_all_traffic ]]
then
	echo "route_for_all_traffic is created"
else
	echo "route_for_all_traffic do not created"
	delete_for_all_proccess        
...
then
	...
	echo "Security group do not activated"
	delete_for_all_proccess        
		exit
fi
