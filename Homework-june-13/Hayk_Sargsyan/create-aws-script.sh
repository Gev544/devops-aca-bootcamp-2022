...
aws ec2 create-vpc \
       ...
       --query Vpc.VpcId --output text > IDs.txt
...
aws ec2 create-subnet \
	...
	--query Subnet.SubnetId --output text >> IDs.txt
...
aws ec2 create-internet-gateway \
	--query InternetGateway.InternetGatewayId --output text >> IDs.txt
...
aws ec2 create-route-table \
	--vpc-id $vpcID \
	--query RouteTable.RouteTableId --output text >> IDs.txt
...
aws ec2 associate-route-table  \
	...
	--route-table-id $rtID \
	--query "AssociationId" --output text >> IDs.txt
...
aws ec2 create-key-pair \
	--key-name aws-homework-key \
	--query "KeyMaterial" --output text > aws-homework-key.pem
...
aws ec2 create-security-group \
	...
	--vpc-id $vpcID \
	--query "GroupId" --output text >> IDs.txt
...
aws ec2 run-instances \
	...
	--subnet-id $subnet1_ID \
	--query 'Instances[*].InstanceId' --output text >> IDs.txt
