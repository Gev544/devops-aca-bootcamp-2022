...
# check if instance exist and rename
...
### remove default route table and create new one
...
### remove default acces control list and create new one
...
conect () {
	...
	# ssh copy nginx script for install on remote
        scp -i "$key_pair_name.pem" ./nginx.sh kyesayan@$instance_ip:/home/kyesayan/nginx.sh
	# install nginx on remoute
        ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo bash /home/ubuntu/nginx.sh"
	# send for_index.html.sh script to remot which will create index.html file for rate Acba usd course
        scp -i "$key_pair_name.pem" ./for_index.html.sh ubuntu@$instance_ip:/home/kyesayan/for_index.html.sh
	# run script for_index.html.sh where is loop
        ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo bash for_index.html.sh > /dev/null 2>&1 &"
	# remove default nginx html file
        ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo rm -rf /var/www/html/index.nginx-debian.html"
        #$instance_ip

	# systemd rate.service create
	scp -i "$key_pair_name.pem" ./rate.service kyesayan@$instance_ip:/home/kyesayan/rate.service

	# move rate.service file
	ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo cp /home/kyesayan/rate.service /etc/systemd/system/rate.service"

	# start rate.service 
	ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo systemctl start rate.service"

	# check status rate.service
	ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo systemctl status rate.service"

	# install awscli
	ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo apt install awscli" -o StrictHostKeyChecking=yes

	# install s3fs
        ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo apt install s3fs"

	# copy credenshals to instance
        scp -i "$key_pair_name.pem" ./.passwd-s3fs kyesayan@$instance_ip:/home/kyesayan/.passwd-s3fs

	# mount bucket
       ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "s3fs kyesayan /home/kyesayan/hp111 -o passwd_file=~/.passwd-s3fs -o use_path_request_style"

       # copy new path to instance
        scp -i "$key_pair_name.pem" ./change_nginx_path kyesayan@$instance_ip:/home/kyesayan/change_nginx_path

	# change nginx path
       ssh -i "$key_pair_name.pem" kyesayan@$instance_ip "sudo mv /home/kyesayan/change_nginx_path  /etc/nginx/sites-available/default"
       
#	ssh -i "$key_pair_name.pem" ubuntu@$instance_ip "sudo s3fs kyesayan /var/www/html -o passwd_file=~/.passwd-s3fs -o dbglevel=info -f -o curldbg -o nonempty -o use_path_request_style"
